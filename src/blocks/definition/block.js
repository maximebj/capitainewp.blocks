...
export default function Block(props) {
  ...
  const getPost = () => {
    apiFetch({ path: `/wp/v2/definitions/${definitionID}` }).then((post) =>
      setDefinition(post),
    )
  }
  ...
  return definition ? (
    <div {...blockProps}>
      <header className="wp-block-capitainewp-definition__header">
        {definition.meta.lesson != "" && (
          <a
            href="#"
            target="_blank"
            className="wp-block-capitainewp-definition__button editor-button button--main"
          >
            Cours dédié
          </a>
        )}
        <div className="wp-block-capitainewp-definition__icon">
          <span className="dashicons dashicons-book-alt"></span>
        </div>
        <p className="wp-block-capitainewp-definition__title">
      ...
      <div className="wp-block-capitainewp-definition__content">
        <p className="wp-block-capitainewp-definition__desc">
      ...
      <div className="wp-block-capitainewp-definition__meta">
        {definition.meta.translation != "" && (
          <p>
            Traduction : <strong>{definition.meta.translation}</strong>
          </p>
        )}
        {definition.meta.abbreviation != "" && (
          <p>
            Abréviation : <strong>{definition.meta.abbreviation}</strong>
          </p>
        )}
      </div>
    </div>
  ) : (
    <Message withSpinner="true" label="Chargement de la définition…" />
  )
}
