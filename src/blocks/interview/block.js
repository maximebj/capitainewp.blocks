...
export default function Block(props) {
  ...
  const getPost = () => {
    apiFetch({ path: `/wp/v2/peoples/${peopleID}` }).then((post) => {
      setPeople(post)

      if (typeof post.meta.photo != "") {
        apiFetch({ path: `/wp/v2/media/${post.meta.photo}` }).then((picture) =>
          setPicture(picture.media_details.sizes.thumbnail.source_url),
        )
      }
    })
  }
  ...
  const isFirstLetterVowel = (name) => {
    return ["a", "e", "i", "o", "u", "y"].indexOf(name[0].toLowerCase()) !== -1
      ? "d'"
      : "de "
  }
  ...
  return people ? (
    <div {...blockProps}>
      ...
      <p className="wp-block-capitainewp-interview__title">
        Le conseil {isFirstLetterVowel(people.title.rendered)}
        <span>{people.title.rendered}</span>
      </p>

      {people.meta.work != "" && (
        <p className="wp-block-capitainewp-interview__work">
          {people.meta.work}
        </p>
      )}
      ...
      <div className="wp-block-capitainewp-interview__meta">
        {people.meta.twitter != "" && (
          <span>
            ...
            <a href="https://twitter.com/{people.meta.twitter}" target="_blank">
              @{people.meta.twitter}
            </a>
          </span>
        )}
        {people.meta.website != "" && (
          <span>
            {" "}
            • Mon site :{" "}
            <a href="{people.meta.website}" target="_blank">
              {people.meta.website_name}
            </a>
          </span>
        )}
      </div>
    ...
    <Message withSpinner="true" label="Chargement de l’interview…" />
  )
}
